...
import zipkin2.reporter.otlp.OtlpSpanHandler;
...
@State(Scope.Thread)
@Threads(1)
public class OtlpSpanHandlerBenchmarks {
  final Reporter<TracesData> NOOP = span -> {

  };
  final SpanHandler handler =
      OtlpSpanHandler.newBuilder(NOOP).alwaysReportSpans(true).build();
  ...
  public static void main(String[] args) throws RunnerException {
    Options opt = new OptionsBuilder()
        .addProfiler("gc")
        .include(".*" + OtlpSpanHandlerBenchmarks.class.getSimpleName() + ".*")
        .build();
